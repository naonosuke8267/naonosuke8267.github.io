...
(() => {
   ...
   * @param {0|1} isRandom 0:non, 1:random rotate
   * @param {Number} speed 0~100
   */
  async function setSpeedSa(characteristic, direction, isRandom, speed) {

    var tempSpeed = speed;
    var tempDirec = direction;
    if( isRandom ){
      tempSpeed = Math.max(tempSpeed, Math.random() * 100);
      tempDirec = Math.round(Math.random());
    }

    const device = characteristic.service.device;
    const idByte = SA_DEVICE_ID_BYTE[device.name];
    const directionAndSpeed = (tempDirec << 7) | tempSpeed;
  ...
  /** @type {HTMLInputElement} */
  const inputIsRandom = document.querySelector('#random');
  inputIsRandom.addEventListener('input', onSelectRandom);

  /** @type {HTMLInputElement} */
  const inputChangeTimeSpan = document.querySelector('#changeTimeSpan');

  async function onInputSpeed() {
    const direction = inputDirection.checked ? 1 : 0;
    const isRandom = inputIsRandom.checked ? 1 : 0;
    const speed = inputSpeed.valueAsNumber;
    await setSpeedSa(characteristic, direction, isRandom, speed);
  }

  async function onSelectRandom() {
    const isRandom = inputIsRandom.checked ? 1 : 0;
    ...
    const direction = inputDirection.checked ? 1 : 0;
    const speed = inputSpeed.valueAsNumber;
    const changeSpan = inputChangeTimeSpan.valueAsNumber;

    await setSpeedSa(characteristic, direction, isRandom, speed);

    setTimeout(() => {
      onSelectRandom();
    }, Math.min(changeSpan, Math.random() * changeSpan));
  }
